...
CREATE OR REPLACE PROCEDURE ADDROLE(p_name IN VARCHAR2) AS
BEGIN
...
CREATE TABLE UserVW (
    ...
    idRole NUMBER,
    token VARCHAR2(200),
    verifiedAccount NUMBER(1,0) DEFAULT 0, -- 1 for true, 0 for false
    CONSTRAINT pk_user PRIMARY KEY (idUser),
    CONSTRAINT fk_user_role FOREIGN KEY (idRole) REFERENCES Role(idRole)
); 

ALTER TABLE UserVW
ADD imageUrl VARCHAR2(255)
ALTER TABLE UserVW
ADD isDelete NUMBER(1,0) DEFAULT 0; -- 1 for true, 0 for false
...
*/
CREATE OR REPLACE PROCEDURE InsertMustAndGetID(
  p_must NUMBER,
  p_id OUT NUMBER
)
AS
BEGIN
  INSERT INTO HourxHour (must) VALUES (p_must) RETURNING idHourXHour INTO p_id;
  COMMIT;
EXCEPTION
  WHEN OTHERS THEN
    ROLLBACK;
...
CREATE TABLE ISSUE (
    ...
    status VARCHAR2(50),
    shift VARCHAR2(1),
    isDelete NUMBER(1,0) DEFAULT 0, -- 1 for true, 0 for false,
    idUser NUMBER,
...
CREATE OR REPLACE PROCEDURE ADDISSUE(
...
*/
CREATE OR REPLACE PROCEDURE UPDATEISSUE(
    p_issueId NUMBER,
    p_HourXHour NUMBER,
    p_idCategory NUMBER,
    p_idType NUMBER,
...
CREATE OR REPLACE PROCEDURE DELETEISSUE(
    p_issueId NUMBER
...
INSERT INTO CATEGORY (name_, description_)
VALUES ('Disponibilidad', 'Categoría relacionada con la disponibilidad de materiales.');

INSERT INTO CATEGORY (name_, description_)
VALUES ('Desempeño', 'Categoría relacionada con el factor humano Proveedor, Logistica.');

INSERT INTO CATEGORY (name_, description_)
VALUES ('Calidad', 'Categoría relacionada con la calidad de retrabajo y desecho de piezas');
...
INSERT INTO TYPES_CATEGORY ( typeCategory, idCategory)
VALUES ('Averias o fallas de equipos(piezas)', 1);

INSERT INTO TYPES_CATEGORY ( typeCategory, idCategory)
VALUES ('Cambio/ajuste Herramientas o modelo(piezas)', 1);

INSERT INTO TYPES_CATEGORY ( typeCategory, idCategory)
VALUES ('Cambio Herramienta de corte(piezas)', 1);

INSERT INTO TYPES_CATEGORY ( typeCategory, idCategory)
VALUES ('Paros Planeados Mantenimiento', 1);
...
INSERT INTO TYPES_CATEGORY ( typeCategory, idCategory)
VALUES ('Desecho y retrabajo (piezas)', 3);
...
select * from IssuesXAvailability
...
CREATE OR REPLACE PROCEDURE ADDESCALATEDISSUE(
...
CREATE OR REPLACE VIEW issueXHourxHour AS
SELECT i.*, tc.typeCategory
FROM ISSUE i
...
BEGIN
     ADDCELL('Célula A', 1, 1); -- Aquí se asumen valores de idUser y idLine
     ADDCELL('Célula B', 2, 2); -- que corresponden a registros existentes
     ADDCELL('Célula C', 3, 3); -- en las tablas de usuarios y líneas
END;
...
RENAME COLUMN mustAcomulative TO mustAccumulative;

ALTER TABLE HourxHour
RENAME COLUMN isAcomulative TO isAccumulative;

ALTER TABLE HourxHour
RENAME COLUMN diference TO difference;

ALTER TABLE HourxHour
RENAME COLUMN diferenceAcomulative TO accumulativeDifference;

ALTER TABLE HourxHour
RENAME COLUMN idCelula TO idCell;

select* from HourxHour
